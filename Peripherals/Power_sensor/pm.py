...
class ATM90E32:
    ...
    def __init__( self, CS_PIN: int = -1, CLK_PIN: int = -1, MISO_PIN: int = -1, MOSI_PIN: int = -1, debug: bool = False ) -> None:
        self.debug = debug
        # Configurando os pinos
        self.MISO   = MISO_PIN if MISO_PIN != -1 else board.D9
        self.MOSI   = MOSI_PIN if MOSI_PIN != -1 else board.D10
        self.CLK    = CLK_PIN if CLK_PIN != -1 else board.D11
        self.CS     = CS_PIN if CS_PIN != -1 else board.D8
        # Configuração do SPI
        self.spi, self.cs = self.configure_spi( clock_divider = 500000 )


    def configure_spi(self, clock_divider: int, bit_order: str = 'MSBFIRST') -> tuple[ busio.SPI, digitalio.DigitalInOut ]:
        # Configuração do SPI
        spi = busio.SPI( clock = self.CLK, MISO = self.MISO, MOSI = self.MOSI )
        # Configuração do CS (Chip Select)
        cs = digitalio.DigitalInOut(self.CS)
        cs.direction = digitalio.Direction.OUTPUT
        # Configuração da ordem dos bits
        # spi.configure( baudrate = clock_divider, polarity = 0, phase = 0, bits = 8, firstbit = self.get_bit_order(bit_order) ) 
        return spi, cs
    ...
    def begin( self, line_freq : int, pga_gain : int, u_gain : int, i_gain_N : int, i_gain_F: int, u_min : int = 190 ):
        ...
        u_sag_thresh = ( u_min * 100 * math.sqrt(2)) / (2 * self.u_gain / 32768)
        ...
        time.sleep( 10 / 1_000_000 )                # 10us de sleep 
    ...
    def read16bits( self, addr : int ) -> int:
        ...
        addr_bytes = bytearray(addr.to_bytes(2, byteorder='big'))
        ...
        response_bytes = bytearray(2)
        self.spi.readinto(response_bytes)
        self.cs.value = True

        # Convertendo os bytes lidos de volta para um valor inteiro
        value = int.from_bytes(response_bytes, byteorder='big')
        return value
